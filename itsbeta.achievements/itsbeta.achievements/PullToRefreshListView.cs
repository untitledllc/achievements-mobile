  ...
  public class PullToRefreshListView : ListView, AbsListView.IOnScrollListener
  {
    ...
    private void ResetHeader()
    {
      if (mRefreshState != TAP_TO_REFRESH)
      {
        ...
        {
            mRefreshViewText.Text = "Нажмите для обновления...";
        }
      ...
      if (mCurrentScrollState == Android.Widget.ScrollState.TouchScroll && mRefreshState != REFRESHING)
      {
        if (firstVisibleItem == 0)
        {
                  ...
                  && mRefreshState != RELEASE_TO_REFRESH)
          {
            ...
            if (AppInfo.IsLocaleRu)
            {
                mRefreshViewText.Text = "Отпустите для обновления...";
            }
            mRefreshViewImage.ClearAnimation();
                ...
                && mRefreshState != PULL_TO_REFRESH)
          {
            ...
            if (AppInfo.IsLocaleRu)
            {
                mRefreshViewText.Text = "Потяните для обновления...";
            }
            if (mRefreshState != TAP_TO_REFRESH)
            {
              mRefreshViewImage.ClearAnimation();
    ...
    public void OnScrollStateChanged(AbsListView view, ScrollState scrollState)
    {
     ...
     public void PrepareForRefresh() {
        ...
        if (AppInfo.IsLocaleRu)
        {
            mRefreshViewText.Text = "Обновление...";
        }
        mRefreshState = REFRESHING;
    }
